...
pdd interPnt(Line l1, Line l2, bool &res){
	...
	tie(p1, p2) = l1;
	tie(q1, q2) = l2;
	double f1 = cross(p2, q1, p1), f2 = -cross(p2, q2, p1), f = (f1 + f2);
	if(fabs(f) < EPS) {
...
bool isin(Line l0, Line l1, Line l2) {// Check inter(l1, l2) in l0
	bool res;
...
vector<Line> halfPlaneInter(vector<Line> lines) {
	...
	sort(ALL(ord), [&](int i, int j) {
		return (abs(ata[i] - ata[j]) < EPS) ? cross(lines[i].S, lines[j].S, lines[i].F) < 0 : ata[i] < ata[j];
	});
	vector<Line> fin;
	for (int i=0; i<sz; i++)
